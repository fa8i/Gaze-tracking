...
def get_face_landmarks_in_ccs(camera_matrix, dist_coefficients, shape, results):
    """
    Proyecta los puntos clave faciales detectados por MediaPipe FaceMesh en el sistema de coordenadas de la cámara.

    Esta función ajusta un modelo 3D de la cara a los puntos clave faciales detectados en 2D en la imagen 
    utilizando `solvePnP`. Estima la rotación y traslación de la cabeza y transforma el modelo facial completo
    al sistema de coordenadas de la cámara.

    Args:
        camera_matrix (np.ndarray): Matriz intrínseca de la cámara.
        dist_coefficients (np.ndarray): Coeficientes de distorsión de la cámara.
        shape (tuple): Dimensiones de la imagen (alto, ancho, canales).
        results (MediaPipe FaceMesh object): Salida de MediaPipe FaceMesh que contiene los puntos clave detectados.

    Returns:
        np.ndarray: Un arreglo 3D que representa los puntos clave faciales transformados en el sistema de coordenadas de la cámara.
    """
    ...
    rvec, tvec = None, None
    success, rvec, tvec, inliers = cv2.solvePnPRansac(face_model, face_landmarks, camera_matrix, dist_coefficients, rvec=rvec, tvec=tvec, useExtrinsicGuess=True, flags=cv2.SOLVEPNP_EPNP)  # Primera aproximacion
    for _ in range(10):
        success, rvec, tvec = cv2.solvePnP(face_model, face_landmarks, camera_matrix, dist_coefficients, rvec=rvec, tvec=tvec, useExtrinsicGuess=True, flags=cv2.SOLVEPNP_ITERATIVE)  # Segunda aproximacion para aumentar exactitud

    head_rotation_matrix, _ = cv2.Rodrigues(rvec.reshape(-1))
    return np.dot(head_rotation_matrix, face_model_all.T) + tvec.reshape((3, 1))  # posiciones 3D de los landmarks
...
def main(base_path: str, output_path: str, screen_height_mm_offset: int = 10):
    fix_qt_cv_mismatch()
    ...
    for idx, row in df.iterrows():
        ...
        print(f"Imagen guardada en: {output_file_path}")
